...
def _fetch_epg_for_channel(
    ...
    Plex returns a MediaContainer XML with <Video> children — NOT pre-formatted
    XMLTV.  We parse each <Video> and convert it to a <programme> block.
    """
    import xml.etree.ElementTree as ET
    ...
    try:
        ...
        # Parse the Plex MediaContainer XML
        try:
            root = ET.fromstring(raw)
        except ET.ParseError:
            return None

        # <Video> elements are direct children of <MediaContainer>
        # (or nested inside <tv channelGridKey="...">)
        ...
        parts = []
        for video in videos:
            prog = _parse_video_to_programme(video, grid_key)
            if prog:
                parts.append(prog)
